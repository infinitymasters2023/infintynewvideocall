...
function ParticipantListItem({ participantId, raisedHand }) {
  const { participant, micOn, webcamOn, displayName, isLocal,enableWebcam, 
    disableWebcam, 
    enableMic, 
    disableMic, } =
  ...
  return (
    <div className="mt-2 m-2 p-2 bg-gray-700 rounded-lg mb-0">
      <div className="flex flex-1 items-center justify-center relative">
        ...
        <div className="m-1 p-1">{micOn ? <MicOnIcon onClick={handleDisableMic}/> : <MicOffIcon onClick={handleEnableMic}/>}</div>
        <div className="m-1 p-1 ">
        {webcamOn ? (
          <>
            <VideoCamOnIcon className='text-base text-white' onClick={handleDisableWebcam} />

            
          </>
        ) : (
          <VideoCamOffIcon onClick={handleEnableWebcam} />
        )}
      </div>
